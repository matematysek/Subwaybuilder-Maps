...
async function initMod() {
    try {
        console.log("Starting DUB mod initialization...");
        const api = await waitForAPI();
        // 1. REGISTER CITIES
        api.registerCity({
            name: "Dublin",
            code: "DUB",
            description: "The capital of Ireland, known for its rich history and vibrant culture.",
            population: 1501000,
            initialViewState: {
                zoom: 13.5,
                latitude: 53.3498,
        ...
        if (api.map.setTileURLOverride) {
            ...
            api.map.setTileURLOverride({
                ...
                tilesUrl: `${mapServerUrl}/DUB/{z}/{x}/{y}.mvt`,
                tileType: 'vector', 
            ...
            api.hooks.onCityLoad((cityCode) => {
                ...
                if (cityCode === 'DUB') {
                    if (api.map.setDefaultLayerVisibility) {
                        api.map.setDefaultLayerVisibility('DUB', {
                            buildingFoundations: false,
                            oceanFoundations: false, 
                            trackElevations: true
                        });
                    }
                    api.ui.showNotification('Dublin loaded successfully!', 'success');
                }
            });
        }
