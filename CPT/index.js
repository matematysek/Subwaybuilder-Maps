...
async function initMod() {
    try {
        console.log("Starting CPT mod initialization...");
        const api = await waitForAPI();
        // 1. REGISTER CITIES
        api.registerCity({
            name: "Cape Town",
            code: "CPT",
            description: "South African Metropolis in the shadow of Table Mountain",
            population: 4770000,
            initialViewState: {
                zoom: 13.5,
                latitude: -34,
        ...
        if (api.map.setTileURLOverride) {
            ...
            api.map.setTileURLOverride({
                ...
                tilesUrl: `${mapServerUrl}/CPT/{z}/{x}/{y}.mvt`,
                tileType: 'vector', 
            ...
            api.hooks.onCityLoad((cityCode) => {
                ...
                if (cityCode === 'CPT') {
                    if (api.map.setDefaultLayerVisibility) {
                        api.map.setDefaultLayerVisibility('CPT', {
                            buildingFoundations: false,
                            oceanFoundations: false, 
                            trackElevations: true
                        });
                    }
                    api.ui.showNotification('Cape Town loaded successfully!', 'success');
                }
            });
        }
