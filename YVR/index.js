...
async function initMod() {
    try {
        console.log("Starting YVR mod initialization...");
        const api = await waitForAPI();
        // 1. REGISTER CITIES
        api.registerCity({
            name: "Vancouver",
            code: "YVR",
            description: "Western Canadas largest city, surrounded by mountains and ocean.",
            population: 2600000,
            initialViewState: {
                zoom: 13.5,
                latitude: 49.2827,
        ...
        if (api.map.setTileURLOverride) {
            ...
            api.map.setTileURLOverride({
                ...
                tilesUrl: `${mapServerUrl}/YVR/{z}/{x}/{y}.mvt`,
                tileType: 'vector', 
            ...
            api.hooks.onCityLoad((cityCode) => {
                ...
                if (cityCode === 'YVR') {
                    if (api.map.setDefaultLayerVisibility) {
                        api.map.setDefaultLayerVisibility('YVR', {
                            buildingFoundations: false,
                            oceanFoundations: false, 
                            trackElevations: true
                        });
                    }
                    api.ui.showNotification('Vancouver loaded successfully!', 'success');
