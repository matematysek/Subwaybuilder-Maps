...
async function initMod() {
    try {
        console.log("Starting FUK mod initialization...");
        const api = await waitForAPI();
        // 1. REGISTER CITIES
        api.registerCity({
            name: "Fukuoka",
            code: "FUK",
            description: "Fukuoka, the largest city on Japan's southern island Kyushu.",
            population: 1600000,
            initialViewState: {
                zoom: 13.5,
                latitude: 33.59,
        ...
        if (api.map.setTileURLOverride) {
            ...
            api.map.setTileURLOverride({
                ...
                tilesUrl: `${mapServerUrl}/FUK/{z}/{x}/{y}.mvt`,
                tileType: 'vector', 
            ...
            api.hooks.onCityLoad((cityCode) => {
                ...
                if (cityCode === 'FUK') {
                    if (api.map.setDefaultLayerVisibility) {
                        api.map.setDefaultLayerVisibility('FUK', {
                            buildingFoundations: false,
                            oceanFoundations: false, 
                            trackElevations: true
                        });
                    }
                    api.ui.showNotification('Fukuoka loaded successfully!', 'success');
                }
            });
        }
