...
async function initMod() {
    try {
        console.log("Starting WAW mod initialization...");
        const api = await waitForAPI();
        // 1. REGISTER CITIES
        api.registerCity({
            name: "Warsaw",
            code: "WAW",
            description: "Polands capital and largest city. It deserves more than 2 metro lines.",
            population: 1860000,
            initialViewState: {
                zoom: 13.5,
                latitude: 52.2297,
        ...
        if (api.map.setTileURLOverride) {
            ...
            api.map.setTileURLOverride({
                ...
                tilesUrl: `${mapServerUrl}/WAW/{z}/{x}/{y}.mvt`,
                tileType: 'vector', 
            ...
            api.hooks.onCityLoad((cityCode) => {
                ...
                if (cityCode === 'WAW') {
                    if (api.map.setDefaultLayerVisibility) {
                        api.map.setDefaultLayerVisibility('WAW', {
                            buildingFoundations: false,
                            oceanFoundations: false, 
                            trackElevations: true
                        });
                    }
                    api.ui.showNotification('Warsaw loaded successfully!', 'success');
                }
            });
        }
