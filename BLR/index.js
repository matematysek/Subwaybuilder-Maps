...
async function initMod() {
    try {
        console.log("Starting BLR mod initialization...");
        const api = await waitForAPI();
        // 1. REGISTER CITIES
        api.registerCity({
            name: "Bengaluru",
            code: "BLR",
            description: "Bengaluru, a fast growing metropolis and the tech capital of India, which basically consists of Universities.",
            population: 18000000,
            initialViewState: {
                zoom: 13.5,
                latitude: 12.9716,
        ...
        if (api.map.setTileURLOverride) {
            ...
            api.map.setTileURLOverride({
                ...
                tilesUrl: `${mapServerUrl}/BLR/{z}/{x}/{y}.mvt`,
                tileType: 'vector', 
            ...
            api.hooks.onCityLoad((cityCode) => {
                ...
                if (cityCode === 'BLR') {
                    if (api.map.setDefaultLayerVisibility) {
                        api.map.setDefaultLayerVisibility('BLR', {
                            buildingFoundations: false,
                            oceanFoundations: false, 
                            trackElevations: true
                        });
                    }
                    api.ui.showNotification('Bengaluru loaded successfully!', 'success');
                }
            });
        }
