...
async function initMod() {
    try {
        console.log("Starting ATH mod initialization...");
        const api = await waitForAPI();
        // 1. REGISTER CITIES
        api.registerCity({
            name: "Athens",
            code: "ATH",
            description: "Extremly chaotic and dense city with a lot of potential...",
            population: 3150000,
            initialViewState: {
                zoom: 13.5,
                latitude: 38,
        ...
        if (api.map.setTileURLOverride) {
            ...
            api.map.setTileURLOverride({
                ...
                tilesUrl: `${mapServerUrl}/ATH/{z}/{x}/{y}.mvt`,
                tileType: 'vector', 
            ...
            api.hooks.onCityLoad((cityCode) => {
                ...
                if (cityCode === 'ATH') {
                    if (api.map.setDefaultLayerVisibility) {
                        api.map.setDefaultLayerVisibility('ATH', {
                            buildingFoundations: false,
                            oceanFoundations: false, 
                            trackElevations: true
                        });
                    }
                    api.ui.showNotification('Athens loaded successfully!', 'success');
                }
            });
        }
